...
class UCloud_PutPolicy
{
    public $CallbackUrl;
    public $CallbackBody;
    public $CallbackMethod;

    public function __construct($callbackUrl, $callbackBody, $callbackMethod="POST")
    {
...
class UCloud_AuthHttpClient
{
    ...
    public function __construct($auth, $mimetype = null, $type = HEAD_FIELD_CHECK, $putPolicy = null)
    {
        $this->Type = $type;
    ...
    public function RoundTrip($req)
    {
        if ($this->Type === HEAD_FIELD_CHECK) {
            $token = $this->Auth->SignRequest($req, $this->MimeType, $this->Type, $this->PutPolicy);

            if ($this->PutPolicy)
                $token = $token . ":" . $this->PutPolicy->encode();
